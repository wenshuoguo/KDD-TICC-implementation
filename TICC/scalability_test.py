...
str_NULL = "VW_data_lam_sparse=" + str(lam_sparse) + "cvxpy_iterations" + str(cvxpy_iterations) + "maxClusters=" + str(
    maxClusters) + "/"
...
for iters in xrange(maxIters):
    ...
    if iters == 0:
        ...
        clustered_points = np.random.choice(num_clusters, size=complete_D_train.shape[0],
                                            replace=True)  # gmm.predict(complete_D_train)
        ...
        clustered_points_kmeans = np.random.choice(num_clusters, size=complete_D_train.shape[0],
                                                   replace=True)  # kmeans.labels_
        kmeans_clustered_pts = np.random.choice(num_clusters, size=complete_D_train.shape[0],
                                                replace=True)  # kmeans.labels_
