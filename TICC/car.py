...
scaling_time = 10
prefix_string = "data_lambda=" + str(lambda_parameter) + "beta = " + str(beta) + "clusters=" + str(
    number_of_clusters) + "/"
...
hexadecimal_color_list = ["000099", "ff00ff", "00ff00", "663300", "996633", "66ffff", "3333cc", "660066", "66ccff",
                          "cc0000", "0000ff", "003300", "33ff00", "00ffcc", "ffff00", "ff9900", "ff00ff", "cccc66",
                          "666666", "ffccff", "660000", "00ff00", "ffffff", "3399ff", "006666", "330000", "ff0000",
                          "cc99ff", "b0800f", "3bd9eb", "ef3e1b"]
...
def compute_BIC_score(actual_clusters, emprical_cov, LLE_all_points_clusters, clustered_points, sorted_training_idx):
    ...
    for cluster in xrange(num_clusters):
        mod_lle += np.log(np.linalg.det(actual_clusters[cluster])) - np.trace(
            np.dot(empirical_cov[cluster], actual_clusters[cluster]))
        ...
        for i in xrange(m):
            if i % scaling_time == 0:
                ...
                if i == m - num_stacked:
                    ...
                    str0 += ","
                    counter += 1
        location_file = open((str_NULL + "location_info_lam_sparse=" + str(lam_sparse) + "threshold=" + str(
            threshold) + "maxClusters=" + str(maxClusters) + ".txt"), "w")
        ...
        if len_train_clusters[cluster] != 0:
            ...
            for block_i in xrange(num_stacked):
                for block_j in xrange(num_stacked):
                    ...
                    lamb[block_i * size_blocks:(block_i + 1) * size_blocks,
                    (block_j * size_blocks):(block_j + 1) * size_blocks] += lambda_block
            ...
            cluster_mean_info[num_clusters, cluster] = np.mean(D_train, axis=0)[
                                                       (num_stacked - 1) * n:num_stacked * n].reshape([1, n])
...
for i in xrange(len(gmm_clustered_pts)):
    if i % scaling_time == 0 and (i < 40000 or i < 5000):
        ...
        if idx == m - num_stacked:
            ...
            str0 += ","
            counter += 1
location_file = open((str_NULL + "EM location file lambda=" + str(lam_sparse) + "beta = " + str(
    switch_penalty) + "clusters=" + str(number_of_clusters) + ".txt"), "w")
location_file.write(str0)
location_file.close()
    ...
    for i in xrange(out.shape[0]):
        for j in xrange(out.shape[1]):
            if np.abs(A[i, j]) > threshold:
                out[i, j] = 1
    file_name = str_NULL + "Cross time graphs" + str(cluster) + "maxClusters=" + str(
        maxClusters - 1) + "lam_sparse=" + str(lam_sparse) + ".jpg"
...
means_file = open((str_NULL + "EM cluster means lam_sparse=" + str(lam_sparse) + "switch_penalty = " + str(
    switch_penalty) + "maxClusters=" + str(maxClusters) + ".txt"), "w")
# means_file.write(str0)
means_file.close()

print "\n\n\nBIC SCORE"
bic = compute_BIC_score(train_cluster_inverse, empirical_cov, LLE_all_points_clusters, clustered_points,
                        sorted_training_idx)
print "BIC:", bic
