...
def generate_inverse(rand_seed):
    ...
    for block_i in xrange(num_blocks):
        for block_j in xrange(num_blocks):
            ...
            if block_i > block_j:
                inv_matrix[block_i * size_blocks:(block_i + 1) * size_blocks,
                block_j * size_blocks:(block_j + 1) * size_blocks] = block_matrices[block_num]
            else:
                inv_matrix[block_i * size_blocks:(block_i + 1) * size_blocks,
                block_j * size_blocks:(block_j + 1) * size_blocks] = np.transpose(block_matrices[block_num])
...
for cluster in xrange(num_clusters):
    ...
    if save_inverse_covarainces:
        np.savetxt("Inverse Covariance cluster =" + str(cluster) + ".csv", cluster_inverses[cluster], delimiter=",",
                   fmt='%1.6f')
        np.savetxt("Covariance cluster =" + str(cluster) + ".csv", cluster_covariances[cluster], delimiter=",",
                   fmt='%1.6f')
...
for counter in xrange(len(break_points)):
    ...
    for num in xrange(old_break_pt, break_pt):
        ...
        # np.savetxt("first point info - conditional covariance.csv", cov_matrix,delimiter = ",", fmt = "%1.4f" )
            ...
            ##The first section
            cov_matrix = cluster_covariances[cluster][0:(num + 1) * size_blocks,
                         0:(num + 1) * size_blocks]  ##the actual covariance matrix
            n = size_blocks
            ...
            if num == 2:
                ...
                print "a is:", a
                print "second term is:", np.dot(np.dot(Sig21, np.linalg.inv(Sig11)),
                                                (a - cluster_mean_stacked[0:(num) * size_blocks, :]))
            ...
            for idx in xrange(num_blocks - 1):
                a[idx * size_blocks:(idx + 1) * size_blocks, 0] = Data[num - num_blocks + 1 + idx, :].reshape(
                    [size_blocks])
            ...
            new_mean = cluster_mean + np.dot(np.dot(Sig21, np.linalg.inv(Sig11)),
                                             (a - cluster_mean_stacked[0:(num_blocks - 1) * size_blocks, :]))
