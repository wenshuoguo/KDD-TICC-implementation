    ...
    if stock_number != -1:
        for i in xrange(n):
            if Matrix_v[stock_number, i] != 0:
                ...
                ax.arrow([time_stock_edge, time_steps * time_separation],
                         [stock_num_edge, stock_number * row_separation], head_width=0.05, head_length=0.1, fc='k',
                         ec='k')  # color ='black', linestyle = '-', linewidth = 0.5)
    else:
        for stock_number in xrange(m):
            for i in xrange(n - time_per_step):
                if Matrix_v[stock_number, i] != 0:
                    ...
                    time_stock_edge = (int(i / time_per_step) + 1) * time_separation
                    ##Draw an edge from the time_step to the
                    plt.plot([time_stock_edge, (time_steps) * time_separation],
                             [stock_num_edge, (stock_number + 1) * row_separation], color='black', linestyle='-',
                             linewidth=0.5)
