...
def computeF1Score_delete(num_cluster, matching_algo, actual_clusters, threshold_algo, save_matrix=False):
    ...
    for cluster in xrange(num_cluster):
        ...
        if save_matrix: np.savetxt("estimated_matrix_cluster=" + str(cluster) + ".csv", estimated_matrix, delimiter=",",
                                   fmt="%1.4f")
        TP = 0
        TN = 0
        FP = 0
...
for iters in xrange(maxIters):
    ...
    plt.figure()
    plt.plot(sorted_test_idx[0:len(clustered_points_test_kmeans)], clustered_points_test_kmeans,
    ...
    binary_Kmeans = (true_confusion_matrix_kmeans[0, 0] + true_confusion_matrix_kmeans[1, 1]) / len(
        clustered_points_test)
